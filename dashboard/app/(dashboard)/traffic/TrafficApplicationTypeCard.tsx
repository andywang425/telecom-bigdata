...
import { getApplicationType } from '@/app/(dashboard)/traffic/actions';
import TrafficApplicationTypeChart, {
  TrafficApplicationTypeChartProps,
} from '@/components/traffic/TrafficApplicationTypeChart';

export default function TrafficApplicationTypeCard() {
  ...
  const [trafficAppData, setTrafficAppData] = React.useState<TrafficResponse.ApplicationType[]>([]);

  useEffect(() => {
    async function fetchData() {
      const res = await getApplicationType(date!.year, date!.month);
      if (res.code === 0) {
        setTrafficAppData(res.data);
      }
    }
  ...
  return (
    <Card variant="outlined" sx={{ p: 2 }}>
      ...
      <CardContent>
        <TrafficApplicationTypeChart data={trafficAppData} valueField={valueField} />
      </CardContent>
      <CardActions>
        <DatePicker
          label={'月份 年份'}
