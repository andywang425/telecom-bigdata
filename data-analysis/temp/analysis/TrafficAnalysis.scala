...
object TrafficAnalysis extends MyLogger {
  def run(spark: SparkSession, trafficDF: DataFrame): Unit = {
    ...
    monthlySessionSummary.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "monthly_session_summary")
      .option("user", "root")
      .option("password", "password")
      .save()

    monthlyUserTrafficSummary.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "monthly_user_traffic_summary")
      .option("user", "root")
      .option("password", "password")
      .save()

    sessionCountByAppType.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "session_count_by_app_type")
      .option("user", "root")
      .option("password", "password")
      .save()

    trafficByAppType.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "traffic_by_app_type")
      .option("user", "root")
      .option("password", "password")
      .save()

    trafficByNetworkTech.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "traffic_by_network_tech")
      .option("user", "root")
      .option("password", "password")
      .save()

    trafficByMonthHour.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "traffic_by_month_hour")
      .option("user", "root")
      .option("password", "password")
      .save()
