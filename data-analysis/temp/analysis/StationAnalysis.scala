...
object StationAnalysis extends MyLogger {
  def run(spark: SparkSession, callDF: DataFrame, smsDF: DataFrame, trafficDF: DataFrame): Unit = {
    ...
    callFailures.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "call_failures")
      .option("user", "root")
      .option("password", "password")
      .save()

    smsFailures.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "sms_failures")
      .option("user", "root")
      .option("password", "password")
      .save()

    baseStationCallStats.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "base_station_call_stats")
      .option("user", "root")
      .option("password", "password")
      .save()

    baseStationSmsStats.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "base_station_sms_stats")
      .option("user", "root")
      .option("password", "password")
      .save()

    baseStationTrafficStats.write.mode("overwrite").format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/telecom_analysis")
      .option("dbtable", "base_station_traffic_stats")
      .option("user", "root")
      .option("password", "password")
      .save()
  }
